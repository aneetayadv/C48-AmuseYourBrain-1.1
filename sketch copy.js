...
function draw() { 
  ...
  if(gameState === 0){
   ...
   /* openDoor(dr1);
    openDoor(dr2);
    openDoor(dr3);*/
// console.log((mousePressedOver(dr1)) || (mousePressedOver(dr3)))
    if((mousePressedOver(dr1)) || (mousePressedOver(dr3))){
      ...
      gameState=2;
      console.log("door2 clicked")
      // dr1.remove()//.visible = false;
      // dr2.remove()//..visible = false;
      // dr3.remove()//..visible = false;
      // r11.remove()//..visible = false;
      // r12.remove()//..visible = false;
      // r13.remove()//..visible = false;
      dr1.visible = false;
      dr2.visible = false;
      dr3.visible = false;
      r11.visible = false;
   ...
   /* openDoor(dr4);
    ...
    if((mousePressedOver(dr5)) || (mousePressedOver(dr6))){
      console.log("Dorr 5 clicked");
      window.answer = "\tDoor-2 Because The Killer is from 1843!!!\t";
      gameState=4;
    }
    else if(mousePressedOver(dr4)){
      gameState=3;
      dr4.remove()//.visible = false;
      dr5.remove()//.visible = false;
      dr6.remove()//.visible = false;
      r21.remove()//.visible = false;
      r22.remove()//.visible = false;
      r23.remove()//.visible = false;
    }
  }
  else if(gameState === 3){
    background(bg3);
   ...
   /* openDoor(dr4);
    ...
    if((mousePressedOver(dr7)) || (mousePressedOver(dr8))){
      window.answer = "\tDoor-1 TEST TEST!!!\t";
      gameState=4;
    }
    else if(mousePressedOver(dr4)){
      window.answer = "CONGRATS YOU WON !!!";
      dr7.remove()//.visible = false;
      dr8.remove()//.visible = false;
      dr9.remove()//.visible = false;
      r21.remove()//.visible = false;
      r22.remove()//.visible = false;
      r23.remove()//.visible = false;
    }
    ...
    console.log("reached gamestate 4")
    background(gameover);
   /* dr1.visible = false;
    dr2.visible = false;
    dr3.visible = false;
